...
def search_activities( 
    ...
    activities: list = get_activities(destination)
    if not activities:
        return f"No activities found for {destination}. Please try a different destination or adjust your search parameters."
    ...
    if budget_max:
        activities = [activity for activity in activities if activity["price"] <= budget_max]
...
def search_restaurants(
    ...
    Args:
        ...
        interests: List of interests like "lunch", "tourist", "groups", "solo dining", "budget" (optional)
    ...
    restaurants: list = get_restaurants(destination)
    if not restaurants:
        return f"No restaurants found for {destination}. Please try a different destination or adjust your search parameters."
    

    # Filter restaurants based on cuisine
    if cuisine:
        restaurants = [restaurant for restaurant in restaurants if cuisine.lower() in restaurant["cuisine"].lower()]
    ...
    if interests:
        ...
        for restaurant in restaurants:
            if any( interest in [bf.lower() for bf in restaurant.get("best_for",[])] for interest in interests_lower):
                filtered.append(restaurant)

            elif any(interest in restaurant.get("cuisine", "").lower() for interest in interests_lower):
                filtered.append(restaurant)
...
#     print(get_activity_recommendations(destination="paris", trip_style="couples", num_days=3))
